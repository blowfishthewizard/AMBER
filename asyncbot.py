			...
			while not self.reader.at_eof():
				...
				if line:
					...
					if args[0] == 'PING':
						...
						_missingchatter()
					elif 'test' in line: # check for test msg in channel
						self._raw('NAMES ' + config.connection.channel0) #send /NAMES query to channel
					elif len(args) >= 4 and args[3] == '=': #if response is long enough to contain names and is direct reply to bot from server
						print("dasit mane ") #we can read the list of users for channel
						#print(args[5:]) #this is the /NAMES response from server as an array with everything removed except nicks
						chan_nicklist = args[5:] #put our list into its own var
						#so now we can run thru names and do whois and our data manipulation etc
						for i in chan_nicklist: 
							self._raw('whois ' + i.replace('@','').replace('+',''))
					elif len(args) == 10 and args[9] == 'time':
						print("checking")
						print(args)
						print(args[3] + " idle for " + args[4] + " seconds")
						daysmissing = str(round(float(args[4]) / 86400, 2))
						if int(args[4]) >= int(172800):#2700
#							self._raw('PRIVMSG ' + config.connection.channel + " :" + '[!] MISSINGCHATTER: ' + args[3] + " lastseen " + args[4] + " seconds.")
...
#							self._raw('PRIVMSG ' + config.connection.channel + " :" + '1,4                                                  ')
#							self._raw('PRIVMSG ' + config.connection.channel + " :" + '1,1                                                  ')
#							self._raw('PRIVMSG ' + config.connection.channel + " :" + '1,0                                                  ')
#							self._raw('PRIVMSG ' + config.connection.channel + " :" + '1,0                                                  ')
#							self._raw('PRIVMSG ' + config.connection.channel + " :" + '1,0  1,1                    1,0                            ')
							self._raw(f'PRIVMSG ' + config.connection.channel + " :" + '1,0  1,1 1,10                  1,1 1,0   12NAME   1: ' + args[3] + '            ')
...
#							self._raw('PRIVMSG ' + config.connection.channel + " :" + '1,0                                                  ')
	...
	async def _missingchatter(self, interval):
		while True:
...
if __name__ == '__main__':
	...
	asyncio.run(Bot._missingchatter(60*1))
